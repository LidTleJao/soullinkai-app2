...
//   // const onUpload = async (personaId: string, f: File)=>{ await uploadImage(personaId, f); };
...
//       <input value={name} onChange={e=>setName(e.target.value)} className="w-full mb-3 px-3 py-2 bg-black/40 border border-white/20 rounded" placeholder="e.g., Luna"/>
//       <label className="block text-sm mb-1">Short description (optional)</label>
//       <input value={desc} onChange={e=>setDesc(e.target.value)} className="w-full mb-4 px-3 py-2 bg-black/40 border border-white/20 rounded" placeholder="e.g., calm strategist"/>

//       <p className="text-sm text-white/70 mb-2">Upload picture after creating on the detail page.</p>
//       <button onClick={submit} className="bg-white text-black px-5 py-2 rounded-full">Create</button>
//     </div>
//   );
// }
