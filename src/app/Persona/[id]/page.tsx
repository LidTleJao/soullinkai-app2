...
import { listPersonas, uploadImage, saveJson } from "../../services/personaService";
...
//   useEffect(()=>{ (async()=>{ if(!id) return; const all=await listPersonas(); setPersona(all.find((x:any)=>x.id===id)||null); })(); },[id]);

//   const sendImage = async (f:File)=>{ await uploadImage(String(id), f); alert("Uploaded."); };
//   const save = async (s:string)=>{ await saveJson(String(id), s, "personality"); alert("Saved JSON"); };
